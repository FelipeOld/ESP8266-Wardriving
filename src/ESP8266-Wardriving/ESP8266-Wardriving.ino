  ...
  if (n == 0) {
    ...
    for (int i = 0; i < n; ++i) {
      if ((WiFi.channel(i) > 0) && (WiFi.channel(i) < 15)) {
        if (!isOnFile(WiFi.BSSIDstr(i))) {
          display.clearDisplay();
          display.setCursor(48, 0);
          sprintf_P(currentTime, PSTR("%02d:%02d"),hour(),minute());
          display.println(currentTime);
          //print line to simplify data view in serial monitor
  ...
  if (logFile) {
    while (logFile.available()) {
      ...
      if (line.indexOf(bssid) != -1) {
        logFile.close();
        return true; // O BSSID já está no arquivo
      }
    }
    logFile.close();
  }
  return false; // O BSSID ainda não foi registrado no arquivo
}
